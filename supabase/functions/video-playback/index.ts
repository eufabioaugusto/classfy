...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: adminRole } = user
      ? await client.from("user_roles").select("role")
        .eq("user_id", user.id).eq("role", "admin").maybeSingle()
      : { data: null };
    const isAdmin = Boolean(adminRole);

    const { data: content } = asset.content_id
      ? await client.from("contents").select(
        "id, creator_id, status, visibility, required_plan, content_type",
      ).eq("id", asset.content_id).single()
      : { data: null };

    const { data: lesson } = !content
      ? await client.from("course_lessons")
        .select(
    ...
    if (content) {
      ...
      } else if (content.creator_id !== user.id && !isAdmin) {
        if (content.visibility === "paid") {
          const { data: purchase } = await client.from("purchased_contents")
            .select("id").eq("user_id", user.id).eq("content_id", content.id)
            .in(
              "status",
          ...
          content.visibility === "pro" || content.visibility === "premium"
        ) {
          const { data: profile } = await client.from("profiles").select("plan")
      ...
      if (!course || course.status !== "approved") {
        ...
        course.creator_id !== user.id && !isAdmin && !lesson.is_preview
      ) {
        if (course.visibility === "paid") {
          const { data: enrollment } = await client.from("course_enrollments")
            .select("id").eq("user_id", user.id).eq("course_id", course.id)
            .maybeSingle();
          if (!enrollment) return json({ error: "Enrollment required" }, 403);
        } else if (
          course.visibility === "pro" || course.visibility === "premium"
        ) {
          const { data: profile } = await client.from("profiles").select("plan")
            .eq("id", user.id).single();
          const rank: Record<string, number> = { free: 0, pro: 1, premium: 2 };
    ...
    const binding = bindings.find((item) =>
      item.id === asset.active_provider_binding_id
    );
