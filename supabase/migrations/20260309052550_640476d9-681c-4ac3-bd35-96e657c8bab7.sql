...
-- Step 1: Alter columns from INTEGER to NUMERIC(10,2)
ALTER TABLE reward_actions_config 
  ALTER COLUMN points_user TYPE NUMERIC(10,2),
  ALTER COLUMN points_creator TYPE NUMERIC(10,2);

ALTER TABLE reward_events 
  ALTER COLUMN points TYPE NUMERIC(10,2);

-- Step 2: Update check_view_milestones function to remove FLOOR()
      ...
      IF OLD.views_count < milestone_config.views AND NEW.views_count >= milestone_config.views THEN
        ...
        IF NOT EXISTS (
          ...
          IF FOUND THEN
            ...
            -- No FLOOR - preserve decimal precision
            points_amt := reward_config.points_creator * plan_mult;
            pp_amt := reward_config.points_creator * plan_mult;
            ...
            -- Accumulate PP in economic_cycle_users
