...
-- ═══════════════════════════════════════════════════════
-- POOL QUALIFICATION — Schema Foundation
-- ═══════════════════════════════════════════════════════

-- 1. wallets: saldo em maturação (bloqueado, pendente de liberação)
ALTER TABLE wallets
  ADD COLUMN pending_balance DECIMAL(10,2) NOT NULL DEFAULT 0;
ALTER TABLE wallets
  ADD CONSTRAINT wallets_pending_nonneg CHECK (pending_balance >= 0);

-- 2. Tabela de saldo pendente de maturação
--    Separada de wallet_transactions para manter ledger imutável
--    Entra em wallet_transactions somente ao maturar
CREATE TABLE public.wallet_pending (
  id                uuid          PRIMARY KEY DEFAULT gen_random_uuid(),
  wallet_id         uuid          NOT NULL REFERENCES wallets(id) ON DELETE CASCADE,
  user_id           uuid          NOT NULL,
  amount            DECIMAL(10,2) NOT NULL CHECK (amount > 0),
  source_type       text          NOT NULL, -- 'pool_distribution'
  cycle_id          uuid          REFERENCES economic_cycles(id) ON DELETE SET NULL,
  idempotency_key   text          UNIQUE,
  mature_at         timestamptz   NOT NULL,
  matured_at        timestamptz,            -- NULL = ainda pendente
...
-- 3. economic_cycle_users: colunas de qualificação
...
-- 4. Expandir platform_settings.economic com defaults completos
...
-- Confirmar resultado
SELECT value FROM platform_settings WHERE key = 'economic';
;
