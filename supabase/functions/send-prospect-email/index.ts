...
function getTemplate(tier: string, channelName: string, niche: string | null) {
  ...
  const templates: Record<string, { subject: string; html: string }> = {
    micro: {
      ...
      html:
        `<div style="font-family:Inter,system-ui,sans-serif;max-width:560px;margin:0 auto;color:#0a0a0a;line-height:1.6">
<p>Oi, tudo bem?</p>
<p>Vi o canal <strong>${channelName}</strong> e queria falar sobre algo que estamos construindo.</p>
<p>A <strong>Classfy</strong> é uma plataforma de conhecimento onde <strong>40% da receita líquida elegível compõe um pool mensal</strong>, distribuído proporcionalmente aos Points válidos do ciclo.</p>
...
<p>A maioria dos criadores${nicheStr} ganha muito menos do que merece. A <strong>Classfy</strong> funciona diferente: <strong>40% da receita líquida elegível compõe um pool mensal</strong>, distribuído proporcionalmente aos Points válidos do ciclo.</p>
...
<p style="font-size:12px;color:#999">Para não receber mais e-mails, <a href="https://classfy.com.br/unsubscribe" style="color:#999">clique aqui</a>.</p>
    ...
    medio: {
      subject:
        `${channelName} — você cria bom conteúdo${nicheStr}. Vamos conversar?`,
      html:
        `<div style="font-family:Inter,system-ui,sans-serif;max-width:560px;margin:0 auto;color:#0a0a0a;line-height:1.6">
<p>Oi,</p>
<p>Acompanhei o <strong>${channelName}</strong> e você claramente sabe criar conteúdo de qualidade${nicheStr}.</p>
<p>Estamos lançando a <strong>Classfy</strong> — <strong>40% da receita líquida elegível compõe um pool mensal</strong>, distribuído proporcionalmente aos Points válidos. O valor definitivo é confirmado no fechamento.</p>
...
<p style="font-size:12px;color:#999">Para não receber mais e-mails, <a href="https://classfy.com.br/unsubscribe" style="color:#999">clique aqui</a>.</p>
    ...
    grande: {
      subject: `${channelName} — uma economia transparente para criadores`,
      html:
        `<div style="font-family:Inter,system-ui,sans-serif;max-width:560px;margin:0 auto;color:#0a0a0a;line-height:1.6">
<p>Oi,</p>
<p>Você já passou pela frustração de ver seu CPM cair de um mês pro outro sem explicação?</p>
<p>Estamos construindo a <strong>Classfy</strong> exatamente por isso. <strong>40% da receita líquida elegível compõe um pool mensal</strong>, distribuído proporcionalmente aos Points válidos do ciclo, por fórmula pública.</p>
...
<p><strong>40% da receita líquida elegível compõe um pool mensal para usuários e Creators, distribuído proporcionalmente aos Points válidos.</strong> Não tem ajuste discricionário nem corte surpresa.</p>
...
serve(async (req) => {
  ...
  try {
    ...
    if (prospect.status !== "pending") {
      throw new Error(`Status inválido: ${prospect.status}`);
    }
