    ...
    if (!content || content.status !== "approved") {
      ...
      if (content.visibility === "paid") {
        const { data: purchase } = await client.from("purchased_contents")
          .select("id").eq("user_id", user.id).eq("content_id", content.id).in(
            "status",
